...
int32_t feeder_period_ticks = 1000000UL / 100;
int32_t brake_period_ticks = 1000000UL / 95;
...
void setup() {
    ...
    PWMC_SetPeriod(PWM, HEAD_FEEDER_PWM_CH, feeder_period_ticks);
    PWMC_SetDutyCycle(PWM, HEAD_FEEDER_PWM_CH, feeder_period_ticks / 2);
    ...
    PWMC_SetPeriod(PWM, HEAD_BRAKE_PWM_CH, brake_period_ticks);
    PWMC_SetDutyCycle(PWM, HEAD_BRAKE_PWM_CH, brake_period_ticks / 2);
...
void loop() {
    ...
    if (spark_state) {
        if (millis() - s_last_tension_control_ms > 100) {
            ...
            if (d < -100) {
                brake_period_ticks = constrain(brake_period_ticks - 10, 2000, 15000);
                PWMC_SetPeriod(PWM, HEAD_BRAKE_PWM_CH, brake_period_ticks);
                PWMC_SetDutyCycle(PWM, HEAD_BRAKE_PWM_CH, brake_period_ticks / 2);
            } else if (d > 100) {
                brake_period_ticks = constrain(brake_period_ticks + 10, 2000, 15000);
